...
class L10nLuGenerateTaxReport(models.TransientModel):
    ...
    def _add_yearly_fields(self, data, form):
        ...
        # 192 and 193 are mandatory if 362 and 363 are present but those are always present so if
        # not presents yet, we add 192 and 193 with the same value as 361 and 362 (probably 0.0).
        if '192' not in form['field_values']:
            form['field_values']['192'] = form['field_values']['361']
        if '193' not in form['field_values']:
