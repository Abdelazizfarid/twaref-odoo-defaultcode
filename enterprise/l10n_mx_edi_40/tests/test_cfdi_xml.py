...
class TestEdiResults(TestMxEdiCommon):
    ...
    def test_xml_invoice_cfdi(self):
        """
        Aim to test if the name of the unit "Units" is translated to UNIDADES in the cfdi xml
        """
        self.env['res.lang']._activate_lang('es_MX')
        mx_lang = self.env['res.lang'].search([['code', '=', 'es_MX']])
        self.env["base.language.install"].create({
            'overwrite': False,
            'lang_ids': [(6, 0, [mx_lang.id])],
        }).lang_install()
        ...
        with freeze_time(self.frozen_today), \
                ...
                patch(
                    'odoo.addons.l10n_mx_edi.models.account_edi_format.AccountEdiFormat._l10n_mx_edi_post_invoice_pac',
                    new=mocked_l10n_mx_edi_pac):
            product_by_unit = self.env['product.product'].create({
                'name': 'my_product_mx',
                'weight': 1,
                'uom_po_id': 1,
                'uom_id': 1,
            ...
            invoice.action_post()
            generated_files = self._process_documents_web_services(invoice)
            cfdi = generated_files[0]
            ...
            expected_etree = self.with_applied_xpath(
                ...
                '''
                    <xpath expr="//Comprobante" position="attributes">
                        <attribute name="CondicionesDePago">Pago inmediato</attribute>
                    </xpath>
                    <xpath expr="//Concepto" position="replace">
                        <Concepto
                            ClaveProdServ="01010101"
