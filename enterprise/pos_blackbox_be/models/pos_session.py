...
class pos_session(models.Model):
    ...
    def _get_pos_ui_product_product(self, params):
        ...
        if self.config_id.iface_fiscal_data_module:
            products = self.env.ref('pos_blackbox_be.product_product_work_in') + self.env.ref('pos_blackbox_be.product_product_work_out')
            # Only load products that are not already in the result
            products = list(set(products.ids) - {product['id'] for product in result})
            context_products = self.env['product.product'].with_context(**params['context'], active_test=False)
            products = context_products.search_read([('id', 'in', products)], fields=params['search_params']['fields'])
