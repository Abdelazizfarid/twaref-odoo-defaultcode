...
@odoo.tests.tagged('-at_install', 'post_install', 'salary', '-standard', 'breaking_16', 'random')
class TestUi(odoo.tests.HttpCase):
    def test_ui(self):
        ...
        extra_days_time_off_type = self.env['hr.leave.type'].create({
            'name': 'Extra Time Off',
        ...
        company_id = self.env['res.company'].create({
            ...
            'hr_contract_timeoff_auto_allocation_type_id': extra_days_time_off_type.id,
        })
        ...
        allocation = self.env['hr.leave.allocation'].search([('employee_id', '=', new_employee_id.id), ('holiday_status_id', '=', extra_days_time_off_type.id)])
