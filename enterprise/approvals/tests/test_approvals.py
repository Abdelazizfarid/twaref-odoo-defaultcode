...
class TestRequest(common.TransactionCase):
    ...
    def test_consistency_between_request_and_approver(self):
        ...
        admin = new_test_user(self.env, login='admin1', groups='base.group_system')
        ...
        # As admin, create an approval
        admin_approval_request = admin_env['approval.request'].create({
        ...
        # As user, create an approval
        user_approval_request = user_env['approval.request'].create({
            'name': 'Test request (user)',
        ...
        # As user, create an approver for your own approval
        user_approver_for_user = user_env['approval.approver'].create({
            'user_id': user.id,
            'request_id': user_approval_request.id
        })

        with self.assertRaises(AccessError):
            user_approver_for_user.request_id = admin_approval_request

        # As admin, create an approver for your own approval with user
        user_approver_for_admin = admin_env['approval.approver'].create({
            'user_id': user.id,
            'request_id': admin_approval_request.id
        })
        # As user, try to move your own approver on your own request
        with self.assertRaises(AccessError):
            user_approver_for_admin.with_env(user_env).request_id = user_approval_request
        # As user, move your own approver on the same request
        user_approver_for_admin.with_env(user_env).request_id = admin_approval_request
        ...
        user_approver.request_id = user_approval_request
