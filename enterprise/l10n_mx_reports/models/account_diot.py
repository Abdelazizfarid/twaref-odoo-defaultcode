...
class MexicanAccountReportCustomHandler(models.AbstractModel):
    ...
    def _execute_query(self, report, current_groupby, options, offset, limit):
        ...
        # Creditable TAX
        tag_8_n = self.env.ref('l10n_mx.tag_diot_8_tax', raise_if_not_found=False) or self.env['account.account.tag']
        tag_8_s = self.env.ref('l10n_mx.tag_diot_8_south_tax', raise_if_not_found=False) or self.env['account.account.tag']
        tag_16 = self.env.ref('l10n_mx.tag_diot_16_tax', raise_if_not_found=False) or self.env['account.account.tag']
        tag_16_imp = self.env.ref('l10n_mx.tag_diot_16_imp_tax', raise_if_not_found=False) or self.env['account.account.tag']
        tag_16_imp_int = self.env.ref('l10n_mx.tag_diot_16_imp_int_tax', raise_if_not_found=False) or self.env['account.account.tag']
        # Creditable BASE
        tag_8_n_base = self.env.ref('l10n_mx.tag_diot_8')
        tag_8_s_base = self.env.ref('l10n_mx.tag_diot_8_south')
        tag_16_base = self.env.ref('l10n_mx.tag_diot_16')
        tag_16_imp_base = self.env.ref('l10n_mx.tag_diot_16_imp')
        tag_16_imp_int_base = self.env.ref('l10n_mx.tag_diot_16_imp_int')
        # Non-creditable TAX
        tag_8_n_nc = self.env.ref('l10n_mx.tag_diot_8_non_cre_tax', raise_if_not_found=False) or self.env['account.account.tag']
        tag_8_s_nc = self.env.ref('l10n_mx.tag_diot_8_south_non_cre_tax', raise_if_not_found=False) or self.env['account.account.tag']
        tag_16_nc = self.env.ref('l10n_mx.tag_diot_16_non_cre_tax', raise_if_not_found=False) or self.env['account.account.tag']
        tag_16_imp_nc = self.env.ref('l10n_mx.tag_diot_16_imp_non_cre_tax', raise_if_not_found=False) or self.env['account.account.tag']
        tag_16_imp_int_nc = self.env.ref('l10n_mx.tag_diot_16_imp_int_non_cre_tax', raise_if_not_found=False) or self.env['account.account.tag']
        # Non-creditable BASE
        tag_8_n_nc_base = self.env.ref('l10n_mx.tag_diot_8_non_cre', raise_if_not_found=False) or self.env['account.account.tag']
        tag_8_s_nc_base = self.env.ref('l10n_mx.tag_diot_8_south_non_cre', raise_if_not_found=False) or self.env['account.account.tag']
        tag_16_nc_base = self.env.ref('l10n_mx.tag_diot_16_non_cre', raise_if_not_found=False) or self.env['account.account.tag']
        tag_16_imp_nc_base = self.env.ref('l10n_mx.tag_diot_16_imp_non_cre', raise_if_not_found=False) or self.env['account.account.tag']
        tag_16_imp_int_nc_base = self.env.ref('l10n_mx.tag_diot_16_imp_int_non_cre', raise_if_not_found=False) or self.env['account.account.tag']
        # Refunds
        ...
        diot_tags = (tag_8_n | tag_8_n_nc | tag_8_s | tag_8_s_nc | tag_16 | tag_16_nc | tag_16_imp | tag_16_imp_nc | tag_16_imp_int | tag_16_imp_int_nc |
                     tag_8_n_base | tag_8_n_nc_base | tag_8_s_base | tag_8_s_nc_base | tag_16_base | tag_16_nc_base | tag_16_imp_base | tag_16_imp_nc_base |
                     tag_16_imp_int_base | tag_16_imp_int_nc_base | tag_exe | tag_exe_imp | tag_0 | tag_no_obj)
        ...
        tag_columns = (
            ...
            'paid_8_n_base', 'paid_8_n_nc_base',
            'paid_8_s_base', 'paid_8_s_nc_base',
            'paid_16_base', 'paid_16_nc_base',
            'paid_16_imp_base', 'paid_16_imp_nc_base',
            'paid_16_imp_int_base', 'paid_16_imp_int_nc_base',
        ...
        self._cr.execute(f"""
            ...
            SELECT
               ...
               sum(raw_results.paid_8_n_base + raw_results.paid_8_n_nc_base) AS paid_8_n_wnc,
               sum(raw_results.paid_8_s_base + raw_results.paid_8_s_nc_base) AS paid_8_s_wnc,
               sum(raw_results.paid_16_base + raw_results.paid_16_nc_base) AS paid_16_wnc,
               sum(raw_results.paid_16_imp_base + raw_results.paid_16_imp_nc_base) AS paid_16_imp_wnc,
               sum(raw_results.paid_16_imp_int_base + raw_results.paid_16_imp_int_nc_base) AS paid_16_imp_int_wnc,
    ...
    def l10n_mx_diot_get_values(self, values, data, partner):
        ...
        # Sums and refunds
        data[7] += float(values['paid_8_n_wnc'])  # 8% Northern +NC paid
        data[8] += float(values['refund_8_n'])  # 8% Northern refunds
        data[9] += float(values['paid_8_s_wnc'])  # 8% Southern +NC paid
        data[10] += float(values['refund_8_s'])  # 8% Southern refunds
        data[11] += float(values['paid_16_wnc'])  # 16% +NC paid
        data[12] += float(values['refund_16'])  # 16% refunds
        data[13] += float(values['paid_16_imp_wnc'])  # 16% imports +NC paid
        data[14] += float(values['refund_16_imp'])  # 16% imports refunds
        data[15] += float(values['paid_16_imp_int_wnc'])  # 16% intangible imports +NC paid
