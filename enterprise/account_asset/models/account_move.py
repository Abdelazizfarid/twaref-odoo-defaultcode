...
class AccountMove(models.Model):
    ...
    def _compute_depreciation_value(self):
        for move in self:
            ...
            if asset:
                ...
                asset_depreciation = sum(depreciation_lines.mapped('balance')) * (-1 if asset.asset_type == 'sale' else 1)
        ...
        for move in self:
            ...
            move.write({'line_ids': [
                Command.update(line.id, {
                    'balance': move.depreciation_value * (-1 if asset.asset_type == 'sale' else 1) * (1 if line in depreciation_lines else -1),
                })
                for line in move.line_ids
            ]})
    ...
    def _get_asset_depreciation_line(self):
        ...
        return self.line_ids.filtered(lambda line: line.account_id.internal_group == account_internal_group or line.account_id == asset.account_depreciation_expense_id)
