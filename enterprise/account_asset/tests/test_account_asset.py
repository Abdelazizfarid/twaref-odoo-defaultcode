...
class TestAccountAsset(TestAccountReportsCommon):
    ...
    def test_negative_asset_balance_inversion(self):
        ...
        def create_asset(asset_type):
            asset = self.env['account.asset'].with_context(asset_type=asset_type).create({
                'name': "Test Asset",
                'original_value': -10000,
        ...
        asset_purchase = create_asset('purchase')
        self.assertRecordValues(asset_purchase.depreciation_move_ids[0].line_ids, [
            {'account_id': asset_account, 'balance': 1000.0},
            {'account_id': expense_account, 'balance': -1000.0},
        ])

        self.assertRecordValues(asset_purchase.depreciation_move_ids[1].line_ids, [
            {'account_id': asset_account, 'balance': 3000.0},
            {'account_id': expense_account, 'balance': -3000.0},
        ])

        asset_expense = create_asset('expense')
        self.assertRecordValues(asset_expense.depreciation_move_ids[0].line_ids, [
            {'account_id': asset_account, 'balance': 1000.0},
            {'account_id': expense_account, 'balance': -1000.0},
        ])

        self.assertRecordValues(asset_expense.depreciation_move_ids[1].line_ids, [
            {'account_id': asset_account, 'balance': 3000.0},
            {'account_id': expense_account, 'balance': -3000.0},
        ])

        asset_sale = create_asset('sale')
        self.assertRecordValues(asset_sale.depreciation_move_ids[0].line_ids, [
            {'account_id': asset_account, 'balance': -1000.0},
            {'account_id': expense_account, 'balance': 1000.0},
        ])

        self.assertRecordValues(asset_sale.depreciation_move_ids[1].line_ids, [
            {'account_id': asset_account, 'balance': -3000.0},
            {'account_id': expense_account, 'balance': 3000.0},
        ])
