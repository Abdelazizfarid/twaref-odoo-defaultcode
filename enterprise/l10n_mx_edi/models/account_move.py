...
class AccountMove(models.Model):
    ...
    def _l10n_mx_edi_get_min_of_now_and_document_date(self):
        now_mx = self._l10n_mx_edi_get_current_mexico_time().replace(tzinfo=None)
        if not self.invoice_date:
            return now_mx

        document_date = datetime.combine(self.invoice_date, time(hour=23, minute=59, second=00))
        return min(now_mx, document_date)
    ...
    def _post(self, soft=True):
        ...
        for move in self:
            if not move.l10n_mx_edi_post_time:
                move.l10n_mx_edi_post_time = move._l10n_mx_edi_get_min_of_now_and_document_date()
