...
class HrContractSalary(main.HrContractSalary):
    ...
    def _get_advantages_values(self, contract):
        ...
        available_cars = request.env['fleet.vehicle'].sudo().search(
            contract._get_available_vehicles_domain(contract.employee_id.address_home_id or contract.employee_id.work_contact_id)).sorted(key=lambda car: car.total_depreciated_cost)
        available_bikes = request.env['fleet.vehicle'].sudo().search(
            contract._get_available_vehicles_domain(contract.employee_id.address_home_id or contract.employee_id.work_contact_id, vehicle_type='bike')).sorted(key=lambda car: car.total_depreciated_cost)
    ...
    def create_new_contract(self, contract, advantages, no_write=False, **kw):
        ...
        if kw.get('package_submit', False):
            ...
            if new_contract.transport_mode_car and not new_contract.new_car:
                available_cars_domain = new_contract._get_available_vehicles_domain(new_contract.employee_id.address_home_id or new_contract.employee_id.address_home_id)
