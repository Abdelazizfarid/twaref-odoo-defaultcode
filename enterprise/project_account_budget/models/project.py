...
class Project(models.Model):
    ...
    def _get_budget_item_data(self, budget_line_info):
        return {
            'name': budget_line_info['general_budget_id'] and budget_line_info['general_budget_id'][1],
            'allocated': budget_line_info['planned_amount'],
            'spent': budget_line_info['practical_amount'],
        }
    ...
    def _get_budget_items(self, with_action=True):
        ...
        if not self.analytic_account_id:
            return {}
        budget_line_domain = self._get_budget_items_domain()
        ...
        for res in budget_line_read_group:
            budget_item = self._get_budget_item_data(res)
            total_allocated += budget_item['allocated']
